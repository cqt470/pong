...
// Configurazione WiFi (cambia con i tuoi dati)
const char* ssid = "FibreBox_X6-347BB7_10";
const char* password = "T9JY4U6CNMC6FMXNW7";

// Configurazione WebSocket
const char* websocket_server = "192.168.1.101";  // IP del server (raspberry/Pc)
...
#define OLED_RESET   -1  // Reset pin (non usato)
#define SCREEN_ADDRESS 0x3C
#define I2C_SDA 21      // Pin SDA ESP32
#define I2C_SCL 22      // Pin SCL ESP32

// Parametri grafici (coerenti con il server)
#define BAR_OFFSET  8
#define BAR_WIDTH   2
#define BAR_HEIGHT  4
#define BALL_SIZE   2
...
struct GameState {
  int l;    // paddle sinistra (posizione Y)
  int r;    // paddle destra
  struct { int x; int y; } b;
} state = { -1, -1, { -1, -1 } };

// UUID casuale (generato una volta)
String uuid;
...
void webSocketEvent(WStype_t type, uint8_t * payload, size_t length) {
  switch(type) {
    ...
    case WStype_CONNECTED:
      ...
      connected = true;
      // Invia UUID per registrarsi come master (slave = false)
...
public:
  ...
  void run() {
    webSocket.loop();
    // Opzionale: heartbeat già gestito dalla libreria
  }
};
